...
// Priority order:
//   1. CSV_FILE env var (explicit override)
//   2. selected_dataset.csv  (generated by the reduction pipeline)
//   3. First CSV found in the directory tree
function discoverCsv(urlDir: string, preferredFile?: string): string {
  ...
  // 2. Prefer the reduced dataset when it exists
  const selected = found.find((f) => path.basename(f) === 'selected_dataset.csv');
  if (selected) return selected;

  // 3. Fall back to the first CSV found
  return found[0]!;
}
...
export const config = {
  ...
  // ── Screenshot capture ────────────────────────────────────────────────────
  // Default: desktop only.  Mobile / fullpage / above-fold are disabled.
  screenshotTypes: (['desktop', 'mobile', 'fullpage', 'above_fold'] as ScreenshotType[]).filter(
    (t) => {
      const map: Record<ScreenshotType, boolean> = {
        ...
        mobile:      envBool('CAPTURE_MOBILE',     false),
        fullpage:    envBool('CAPTURE_FULLPAGE',   false),
        above_fold:  envBool('CAPTURE_ABOVEFOLD',  false),
      };
      return map[t];
    },
  ),
  ...
  // ── Page discovery ────────────────────────────────────────────────────────
  // Controls which sub-page types are discovered and captured per URL.
  pageDiscovery: {
    login:           envBool('DISCOVER_LOGIN',           true),
    signup:          envBool('DISCOVER_SIGNUP',          true),
    forgotPassword:  envBool('DISCOVER_FORGOT_PASSWORD', true),
    checkout:        envBool('DISCOVER_CHECKOUT',        true),
    payment:         envBool('DISCOVER_PAYMENT',         true),
    account:         envBool('DISCOVER_ACCOUNT',         true),
    profile:         envBool('DISCOVER_PROFILE',         true),
    dashboard:       envBool('DISCOVER_DASHBOARD',       true),
  },
  ...
  // Maximum secondary pages to discover per URL (caps memory + time per URL)
  maxSubPagesPerUrl: envNum('MAX_SUBPAGES_PER_URL', 5),
